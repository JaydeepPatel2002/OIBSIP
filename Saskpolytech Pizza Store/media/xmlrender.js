...
function load_quotes() {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            myxml = xhttp.responseText
            var xmlDoc  = parseXml(myxml)
            if(xmlDoc) {
                var strHtml = "";
                for(q=0;q<xmlDoc.getElementsByTagName("Quote").length;q++) {
                    var quotes = xmlDoc.getElementsByTagName("Quote")[q].childNodes;
                    strHtml+="<blockquote>"
    ...
    xhttp.open("GET", "media/quotes.xml", true);
    xhttp.send();
}
